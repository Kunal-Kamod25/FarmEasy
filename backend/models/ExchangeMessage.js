...
class ExchangeMessage {
  ...
  static async create(match_id, sender_id, message) {
    ...
    const [result] = await db.query(query, [match_id, sender_id, message]);
  ...
  static async getByMatchId(match_id) {
    ...
    const [results] = await db.query(query, [match_id]);
    return results;
  }
  ...
  static async markAsRead(match_id, reader_id) {
    ...
    const [result] = await db.query(query, [match_id, reader_id]);
    return result;
  }
  ...
  static async searchInMatch(match_id, keyword) {
    ...
    const [results] = await db.query(query, [match_id, `%${keyword}%`]);
    return results;
  }
}
