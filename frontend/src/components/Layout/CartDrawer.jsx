...
const CartDrawer = ({ drawerOpen, toggleCartDrawer }) => {
  ...
  useEffect(() => {
    ...
    return (
        ...
        className={'fixed top-0 right-0 w-3/4 sm:w-1/2 md:w-1/4 h-full bg-[#FFFFF0] shadow-lg transform transition-transform duration-300 flex flex-col z-50 ' + (drawerOpen ? 'translate-x-0' : 'translate-x-full')}>
            
        {/* Close button */}
        <div className="flex justify-end p-4">
            <button onClick={toggleCartDrawer} >
                <IoMdClose className="h-6 w-6 text-black hover:text-green-700"/>
            </button>
        </div>
